...
void explore(void) {
	...
	LPose_t initialPose = ownLaby_getPose();

    struct position {
        int x;
        int y;
    } current_position = {initialPose.column, initialPose.row};
    
    struct position target_tile = {0,0};


    while (robot_canContinue()) {
        ...
        if (exitDir > -1) {
            ...
            if (pose.cardinalDirection == DIRECTION_NORTH){
            	if (canMoveForward == true) {
	            	...
	            	target_tile.x = current_position.x;
	            	target_tile.y = current_position.y+1;
	            }
            	if (canMoveLeft == true){
	            	if (lowestVisitCount > visit_count[current_position.x-1][current_position.y]){
	            		...
	            		target_tile.x = current_position.x-1;
	            		target_tile.y = current_position.y;
	            	}
            	}
             ...
             current_position.x = target_tile.x;
             current_position.y = target_tile.y;
             //fprintf(stderr, "\n");
